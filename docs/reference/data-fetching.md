...
In an ideal world the HubSpot GraphQL integration would be the go to for getting all of your HubSpot content into the JS Building Blocks. Currently however, GraphQL only supports querying HubSpot CRM, HubDB, and blog data - refer to the [GraphQL](/reference/js-modules.html#graphql) documentation elsewhere. There are some key advantages to using the GraphQL integration with JS Building Blocks
